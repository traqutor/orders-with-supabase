...
export function useOrderPins() {
  const url = '/api/dashboard/pinned_orders';


  const pinOrder = async (payload: NewPinnedOrder) => {
    const response = await postData({ url: `${url}/${payload.user_id}`, data: payload });

    if (response.error) throw new Error(` Pin Order: ${payload} : ${JSON.stringify(response)}`);
  ...
  const unPinOrder = async (payload: PinnedOrder) => {
    ...
    if (response.error) throw new Error(`Delete Pin: ${payload} Error: ${JSON.stringify(response)}`);
